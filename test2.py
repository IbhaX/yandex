...
class YandexMarketReviews:
    ...
    def fetch_reviews(self, product_link, item):
        print(f"Scraping reviews for {product_link}...")
        page = 1
        while True:
            ...
            if response.status_code == 200:
                ...
                if not reviews:
                    print(f"No reviews found for {product_link}.")
                ...
                print(f"Failed to scrape page {page} for {product_link}. Status code: {response.status_code}")
    ...
    def run(self):
        for item in self.items:
            self.fetch_reviews(item['product_link'], item)
